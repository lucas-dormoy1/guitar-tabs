...
import { GrilleMesure, type Selection } from "./GrilleMesure";
...
type Props = {
  ...
  onChanger: (chanson: ChansonArpege) => void;
};

export function EditeurArpege({ chanson, onChanger }: Props) {
  ...
  const toucherCase = (corde: Corde, pas: number) => {
    ...
    if (dejaActive) {
      if (trouverNote(mesure, corde, pas)) {
        onChanger(retirerNote(chanson, indexSection, indexMesure, corde, pas));
      }
      setSelection(null);
      return;
    }
  ...
  const effacerNote = () => {
    ...
    onChanger(retirerNote(chanson, indexSection, indexMesure, selection.corde, selection.pas));
    setSelection(null);
  };
            ...
            {PAS_POSSIBLES.map((pas) => {
              ...
              return (
                <Pressable
                  key={pas}
                  onPress={() => {
                    onChanger(changerPasMesure(chanson, indexSection, indexMesure, pas));
                    setSelection(null);
                  }}
